...
function displayVideo(responseData,isSort=false){
    ...
    for(const item of responseData){
        ...
        videoCard.innerHTML=`
        <div class="relative">
            <div class="w-[312px] h-[200px] rounded-lg sm:w-[400px] md:w-[312px]">
                <img class="w-[312px] h-[200px] rounded-lg sm:w-[400px] md:w-[312px]" src="${item.thumbnail}" alt="${item.title}" >
            </div>
            ${time.years > 0 ? `<div id="time" class=" bg-[#171717] rounded-sm h-5  w-[90px] text-[#fff] text-[10px] absolute left-[213px] bottom-[12px] sm:left-[290px] md:left-[213px]">
                <p id="time-text" class="text-center pt-[2px]"><span>${time.years}</span>years ago</p>
                </div>`: time.months > 0 ? `<div id="time" class=" bg-[#171717] rounded-sm h-5  w-[90px] text-[#fff] text-[10px] absolute left-[213px] bottom-[12px] sm:left-[290px] md:left-[213px]">
                <p id="time-text" class="text-center pt-[2px]"><span>${time.months}</span>months ago</p>
                </div>`: time.days > 0 ? `<div id="time" class=" bg-[#171717] rounded-sm h-5  w-[90px] text-[#fff] text-[10px] absolute left-[213px] bottom-[12px] sm:left-[290px] md:left-[213px]">
                <p id="time-text" class="text-center pt-[2px]"><span>${time.days}</span>days ago</p>
                </div>`: time.hours > 0 ? `<div id="time" class=" bg-[#171717] rounded-sm h-5  w-[90px] text-[#fff] text-[10px] absolute left-[213px] bottom-[12px] sm:left-[290px] md:left-[213px]">
                <p id="time-text" class="text-center pt-[2px]"><span>${time.hours}</span>hrs <span>${time.minutes}</span>min ago</p>
                </div>`: time.minutes > 0?`<div id="time" class=" bg-[#171717] rounded-sm h-5  w-[90px] text-[#fff] text-[10px] absolute left-[213px] bottom-[12px] sm:left-[290px] md:left-[213px]">
                <p id="time-text" class="text-center pt-[2px]"><span>${time.minutes}</span>minutes <span>${time.seconds}</span>sec ago</p>
                </div>`:time.seconds > 0  ?`<div id="time" class=" bg-[#171717] rounded-sm h-5  w-[90px] text-[#fff] text-[10px] absolute left-[213px] bottom-[12px] sm:left-[290px] md:left-[213px]">
                <p id="time-text" class="text-center pt-[2px]"><span>${time.seconds}</span>sec ago</p>
                </div>`:''
            }
...
const allCateogory = async() => {
    ...
    if(navigator.onLine){
        contronLoading(true);
        document.getElementById('oflineLoading').classList.add('hidden');
        document.getElementById('oflineImage').classList.add('hidden');
    }
    else{
        oflineLoading(); 
...
function oflineLoading() {
    ...
    const noData = document.getElementById('no-data-view');
    if (!navigator.onLine && off === 0) {
        off = 1;
        noData.classList.add('hidden');
        console.log('helloooooooo');
        const ofLine = document.getElementById('oflineLoading');
        const ofImage = document.getElementById('oflineImage');

        ofLine.classList.remove('hidden');
        setTimeout(function () {
            console.log('time outttttttttt');
            ofLine.classList.add('hidden');
            ofImage.classList.remove('hidden');
        }, 10000);
